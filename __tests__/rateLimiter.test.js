...
describe("Rate Limiter Security Tests", () => {
  test("should prevent rapid fire attacks", async () => {
    ...
    // Simulate rapid fire attack (20 requests quickly)
    for (let i = 0; i < 20; i++) {
      try {
        await applyRateLimit(request, "auth"); // limit: 5 per 15 minutes
      } catch (error) {
        if (error.status === 429) {
          errorCount++;
        }
    ...
    // Should have blocked most requests (20 - 5 = 15 blocked)
    expect(errorCount).toBe(15);
  });

  test("should handle different IP addresses independently", async () => {
    const request1 = createMockRequest("192.168.1.101");
    const request2 = createMockRequest("192.168.1.102");

    // Use up limit for first IP
    for (let i = 0; i < RATE_LIMITS.upload.limit; i++) {
      await applyRateLimit(request1, "upload");
    }

    // First IP should be blocked
    await expect(applyRateLimit(request1, "upload")).rejects.toThrow();

    // Second IP should still work
    const result = await applyRateLimit(request2, "upload");
    expect(result.headers["X-RateLimit-Remaining"]).toBe("2"); // 3 - 1
  });
});
